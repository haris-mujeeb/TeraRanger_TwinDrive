...
WiFiClient client;
WiFiServer server(serverPort);  // Create a server that listens on port 80
...
void forwardTelemetryData() {
  ...
  if ( (client && client.connected()) && (millis() - lastTime > 10)) {
    ...
    String data = String(teleData.robotYawDegrees) + "," + String(teleData.robotDistanceCm) + "\n";
      
    client.println(data);   // Send data to the Python server when requested
