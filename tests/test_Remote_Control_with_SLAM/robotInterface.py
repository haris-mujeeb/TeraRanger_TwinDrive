...
class CustomPlotWidget(pg.PlotWidget):
    ...
    def __init__(self, robot_interface, parent=None):
        ...
        self.turn_angle = 10  # Example value (degrees), adjust as needed
...
class RobotInterface:
    ...
    def __init__(self, host_receive: str, port_receive: int, host_send: str, port_send: int):
        ...
        self.plot_history_length = 500 # Example: Keep last 500 points visible on plot
        self._robot_x_history = collections.deque([0.0], maxlen=self.plot_history_length)
        self.robot_y_history = collections.deque([0.0], maxlen=self.plot_history_length)
        ...
        self.path_curve = self.plot_widget.plot(
            self._robot_x_history, self.robot_y_history,
        ...
        self.DISTANCE_TOLERANCE_MM = 15.0 # Millimeters
    ...
    def _save_sensor_data_to_csv(self, filename: str = 'sensor_data.csv'):
        ...
        try:
            ...
            with open(filename, mode='a', newline='') as file:
                ...
                if not file_exists and (len(self.tof_sensor_values) > 0 or len(self._robot_sensor_values) > 0):
                    header = ['Timestamp'] + \
    ...
    def _calculate_points_for_plot(self) -> bool:
        ...
        with self._data_lock:
            ...
            if self._robot_x_history == [0.0] and self.robot_y_history == [0.0] and self.prev_distance == 0.0:
            ...
            self._robot_x_history.append(self._robot_x_history[-1] + dx)
            self.robot_y_history.append(self.robot_y_history[-1] + dy)
            self.logger.debug(f"Robot position calculated: X={self._robot_x_history[-1]:.2f}, Y={self.robot_y_history[-1]:.2f}")
            ...
            if len(self.tof_sensor_values) > 0:
                ...
                if len(valid_distances) > 0:
                    ...
                    end_y_coords = self.robot_y_history[-1] + valid_distances * np.sin(valid_angles) * np.cos(pitch_angle_rad)
            ...
            self.logger.debug(f"📊 Plot data prepared. Robot: ({self._robot_x_history[-1]:.2f}, {self.robot_y_history[-1]:.2f}), ToF Map Points: {len(self._tof_data_history)}")
            return True
    ...
    def _update_plot(self):
        ...
        self.path_curve.setData(self._robot_x_history, self.robot_y_history)

        # Update current robot position marker
        if self._robot_x_history: # Check if points exist before accessing last element
          x, y = self._robot_x_history[-1], self.robot_y_history[-1]
          ...
          if (x != self._robot_x_history[-2] or y != self.robot_y_history[-2] or angle_deg != self.prev_angle):
    ...
    def _handle_key_press(self, event: QtGui.QKeyEvent, move_distance: float, turn_angle: float, speed: float) -> bool:
        ...
        """
        current_distance_in_mm = self._robot_sensor_values[1]
    ...
    def _start_new_move_thread(self, target_x: float, target_y: float, speed: float):
        ...
        self.logger.info(f"Starting new move thread to Target: X={target_x:.2f}mm, Y={target_y:.2f} at speed {speed}")
    ...
    def move_to_xy(self, target_x: float, target_y: float, speed: float):
        ...
        """
        self.logger.info(f"Move to target requested via mouse click: X={target_x:.2f}mm, Y={target_y:.2f}mm at speed {speed}")
        # Start the movement in a new thread to avoid blocking the GUI
        self._start_new_move_thread(target_x, target_y, speed)
    ...
    def _move_to_target(self, target_x: float, target_y: float, speed: float):
        ...
        """
        self.logger.info(f"Executing movement to Target: X={target_x:.2f}mm, Y={target_y:.2f}mm at speed {speed}")
        ...
        current_y = self.robot_y_history[-1] if self.robot_y_history else 0.0
        ...
        if distance_to_target_in_mm > self.DISTANCE_TOLERANCE_MM: # Only move if significant distance
            initial_robot_distance_in_mm = self._robot_sensor_values[1] # Assuming index 1 is distance
    ...
    @property
    def _robot_x_position_history(self):
